...
if (!isset($_GET['StaffICPassport']) || empty($_GET['StaffICPassport'])) {
    ...
    echo "<script>window.location.href='manage-ftw.php'</script>";
    exit;
}
                ...
                <div class="tile">
                    ...
                    <div class="table-container">
                        <form method="post" action="">
                            ...
                            <h4 align="">Assessment Details</h4>
                            <div class="table-container">
                                <table class="table table-hover table-bordered" id="sampleTable">
                                    ...
                                    <tbody>
                                        <?php
                                        $lastPlaceExam = ""; // To track the last place_exam to group the rows
                                        foreach ($assessments as $assessment): 
                                            // Group by place_exam
                                            if ($lastPlaceExam != $assessment->place_exam):
                                                ?>
                                                <tr>
                                                    <td colspan="5" style="background-color: #f1f1f1; font-weight: bold;">
                                                        <?php echo htmlentities($assessment->place_exam); ?>
                                                    </td>
                                                </tr>
                                                <?php
                                                $lastPlaceExam = $assessment->place_exam;
                                            endif;
                                        ?>
                                        <tr>
                                            <td><?php echo htmlentities($assessment->place_exam); ?></td>
                                            <td><?php echo htmlentities($assessment->date_exam); ?></td>
                                            <td><?php echo htmlentities($assessment->JobSpecific); ?></td>
                                            <td class="<?php echo $assessment->status_ftw == 'Fit' ? 'result-pass' : ($assessment->status_ftw == 'Unfit' ? 'result-fail' : 'result-pending'); ?>">
                                                <?php echo htmlentities($assessment->status_ftw); ?>
                                            </td>
                                            <td>
                                                <a href="edit-ftwdetailadd.php?StaffICPassport=<?= htmlentities($staffICPassport); ?>&assessment_id=<?= htmlentities($assessment->result_id); ?>" class="btn btn-success">
                                                <i class="bi bi-pencil-square"></i> Edit
                                                </a>

                                                <a href="edit-ftw.php?StaffICPassport=<?= htmlentities($staffICPassport); ?>&del=<?= htmlentities($assessment->result_id); ?>" 
                                                onclick="return confirm('Are you sure you want to delete this record?');" class="btn btn-danger">
